class IncomingController < ApplicationController
  ...
  def create
    ...
    if user_nil?(user_from_email)
      create_new_user(user_from_email, new_user_created)
    end
    ...
    send_password_reset?(current_user, new_user_created)
    
    head 200
  end

  def user_nil?(user_from_email)
    User.find_by(email: user_from_email)
  end 
  ...
  def create_new_user(user_from_email, new_user_created)
  ...
  def send_password_reset?(current_user, new_user_created)
    if new_user_created 
